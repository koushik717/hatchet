...
By default, the Go SDK retries certain client calls with exponential backoff. REST reads use up to 5 total attempts: the initial attempt plus up to 4 retries. gRPC calls use up to 5 attempts with a 30 second per-retry timeout.

REST read retries use a 250ms base delay, a factor of 2, full jitter, and a 5 second per-delay cap. When the caller request context has no deadline, each REST attempt waits up to 30 seconds for response headers. The per-attempt header timeout does not cut off response body reads. If the caller context already has a deadline, that deadline governs the whole request.
...
For HTTP 429 responses, the Go SDK honors a `Retry-After` header only when it is valid and no greater than 5 seconds. Otherwise it falls back to jittered exponential backoff.
...
| `HATCHET_CLIENT_NO_GRPC_RETRY` | Legacy gRPC-only switch. Disables gRPC SDK retries only. REST read retries remain enabled unless `HATCHET_CLIENT_NO_RETRY` is set. |
