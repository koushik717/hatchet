...
func newFromOpts(opts *ClientOpts) (Client, error) {
	...
	grpcRetryEnabled, restRetryEnabled := sdkRetryEnabled(opts.noRetry, opts.noGrpcRetry)
...
func sdkRetryEnabled(noRetry, noGrpcRetry bool) (grpcRetryEnabled, restRetryEnabled bool) {
	grpcRetryEnabled = !noRetry && !noGrpcRetry
	restRetryEnabled = !noRetry
	return grpcRetryEnabled, restRetryEnabled
}

func newRestHTTPDoer() retry.HTTPDoer {
	return retry.NewRestDoer(&http.Client{})
}
