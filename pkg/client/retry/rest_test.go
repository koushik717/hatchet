...
func newTestRestDoer(inner HTTPDoer) (*RestDoer, chan time.Duration) {
	...
	return NewRestDoer(inner, WithRestClock(time.Now, sleep, rand.New(rand.NewPCG(1, 1)).Int64N)), slept
}
...
func TestRestDoerDoesNotRetryNonIdempotentRequests(t *testing.T) {
	t.Parallel()
...
func TestRestDoerPerAttemptTimeoutWithoutCallerDeadline(t *testing.T) {
	t.Parallel()
	...
	doer := NewRestDoer(server.Client())
...
func TestRestDoerCallerDeadlineWins(t *testing.T) {
	t.Parallel()
...
func TestRestDoerFinalBodyReadableAfterClose(t *testing.T) {
	t.Parallel()
